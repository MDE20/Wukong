...
func ReadGraphAndSeen(fname string) ([]gnnlib.Edge, map[int]bool) {
	var x GraphWithSeen
	wukong.ReadJSON(fname, &x)
	return x.Graph, x.Seen
}

func Exec(ppCfg wukong.PreprocessConfig, modelCfg wukong.ModelConfig, plan wukong.PlannerConfig, execCfg wukong.ExecConfig) {
	predFunc := predicate.GetPredicate(ppCfg.Predicate)
	log.Printf("[exec] loading train tracks")
	var trainTracks [][]wukong.Detection
	...
	for i, track := range trainTracks {
		trainLabels[i] = predFunc([][]wukong.Detection{track})
	}
	...
	if _, err := os.Stat(execCfg.TrackOutput); err != nil {
		...
		wukong.WriteJSON(execCfg.TrackOutput, GraphWithSeen{graph, seenFrames})
	} else {
	...
	if _, err := os.Stat(execCfg.FilterOutput); err != nil {
		...
		wukong.WriteJSON(execCfg.FilterOutput, components)
	} else {
		log.Printf("[exec] read filter output")
		wukong.ReadJSON(execCfg.FilterOutput, &components)
	}
	...
	if _, err := os.Stat(execCfg.UncertaintyOutput); err != nil {
		...
		wukong.WriteJSON(execCfg.UncertaintyOutput, GraphWithSeen{graph, seenFrames})
	} else {
	...
	if _, err := os.Stat(execCfg.RefineOutput); err != nil {
		...
		for _, r := range refiners {
			...
			for iter := 0; ; iter++ {
				...
				components := gnn.GetComponents(graph)
				components = filterComponents(components)
				inTracks := make([][]wukong.Detection, len(components))
		...
		wukong.WriteJSON(execCfg.RefineOutput, GraphWithSeen{graph, seenFrames})
	} else {
	...
	wukong.WriteJSON(execCfg.OutPath, wukong.TracksToDetections(tracks))
