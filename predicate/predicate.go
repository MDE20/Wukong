...
type Predicate func(tracks [][]wukong.Detection) bool
...
func StartEndPredicate(poly1 common.Polygon, poly2 common.Polygon) Predicate {
	return func(tracks [][]wukong.Detection) bool {
		track := tracks[0]
		if len(track) == 0 {
			return false
		}
...
func GetPredTime(track []wukong.Detection, idx int, nframes int) int {
	for i := idx - 1; i >= 0; i-- {
...
func GetPredDistance(track []wukong.Detection, idx int, distance float64) int {
	for i := idx - 1; i >= 0; i-- {
