...
eprint("123")
eprint(detection_path)
frame_path = sys.argv[3]
frame_scale = int(sys.argv[4])
# 检查检查点文件是否存在
if os.path.exists(model_path + ".index") and os.path.exists(model_path + ".data-00000-of-00001"):
    eprint("Checkpoint files exist")
else:
    eprint("Checkpoint files are missing or path is incorrect")
    exit(1)
...
try:
    m.saver.restore(session, model_path)
    eprint("Model restored successfully")
except ValueError as e:
    eprint("Error restoring checkpoint:", str(e))
    exit(1)
# # 设置 GNN 模型检查点目录
# gnn_model_dir = 'logs/shibuya/gnn/'

# # 获取最新的 GNN 检查点路径
# latest_gnn_checkpoint = tf.train.latest_checkpoint(gnn_model_dir)

# if latest_gnn_checkpoint:
#     # 初始化 GNN 模型
#     m = model.Model([[input_example], [target_example]])
...
#     # 恢复 GNN 模型
#     eprint(f"Restored GNN model from checkpoint: {latest_gnn_checkpoint}")
#     m.saver.restore(session, latest_gnn_checkpoint)
#     eprint("GNN Model restored from checkpoint")
# else:
#     eprint(f"No GNN checkpoint found in {gnn_model_dir}")

# # 设置 RNN 模型检查点目录
# filter_rnn_checkpoint_dir = 'logs/shibuya/10/filter-rnn'
# refine_rnn_checkpoint_dir = 'logs/shibuya/10/refine-rnn'


# # 获取最新的 RNN 检查点路径
# latest_filter_checkpoint = tf.train.latest_checkpoint(filter_rnn_checkpoint_dir)
# latest_refine_checkpoint = tf.train.latest_checkpoint(refine_rnn_checkpoint_dir)

# reader = tf.train.NewCheckpointReader(latest_filter_checkpoint)
# var_to_shape_map = reader.get_variable_to_shape_map()
# for key in var_to_shape_map:
#     eprint(f"tensor_name1: {key}")

# reader = tf.train.NewCheckpointReader(latest_refine_checkpoint)
# var_to_shape_map = reader.get_variable_to_shape_map()
# for key in var_to_shape_map:
#     eprint(f"tensor_name2: {key}")

# eprint(f"Latest filter RNN checkpoint: {latest_filter_checkpoint}")
# eprint(f"Latest refine RNN checkpoint: {latest_refine_checkpoint}")

# if latest_filter_checkpoint:
#     try:
#         m.saver.restore(session, latest_filter_checkpoint)
#         eprint(f"Restored filter RNN model from checkpoint: {latest_filter_checkpoint}")
#     except ValueError as e:
#         eprint(f"Error restoring filter RNN checkpoint: {e}")
# else:
#     eprint("No filter RNN checkpoint found, model was not restored.")

# if latest_refine_checkpoint:
#     try:
#         m.saver.restore(session, latest_refine_checkpoint)
#         eprint(f"Restored refine RNN model from checkpoint: {latest_refine_checkpoint}")
#     except ValueError as e:
#         eprint(f"Error restoring refine RNN checkpoint: {e}")
# else:
#     eprint("No refine RNN checkpoint found, model was not restored.")
