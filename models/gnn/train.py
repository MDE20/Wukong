...
log_dir = '../../logs/' + dataset + '/gnn/tensorboard/' + datetime.now().strftime("%Y%m%d-%H%M%S")
...
for epoch in range(9999):
    ...
    for _ in range(1024):
        ...
        feed_dict.update(d1)
        feed_dict.update(d2)
        _, loss, summary = session.run([m.optimizer, m.loss, summary_op], feed_dict=feed_dict)
        train_losses.append(loss)
    train_loss = numpy.mean(train_losses)
    train_time = time.time()

	# Log training summary once per epoch
    ...
    for i in range(0, len(val_pairs), model.BATCH_SIZE):
        ...
        feed_dict.update(d1)
        feed_dict.update(d2)
        loss, outputs, summary = session.run([m.loss, m.outputs, summary_op], feed_dict=feed_dict)
        val_losses.append(loss)

    val_loss = numpy.mean(val_losses)
    val_time = time.time()

	# Log validation summary once per epoch
